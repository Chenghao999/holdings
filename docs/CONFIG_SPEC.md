...
> **标了「尚未生效」的 2 项请勿依赖**：它们会被正常解析并保留在配置对象上，
> 但没有任何代码读取，改动不会产生任何行为变化。降级的真实逻辑见
> [FAQ](FAQ.md#数据同步)。这 2 项是作为结构预留保留的，接线工作见
> [ROADMAP](ROADMAP.md)。
>
> 超时的语义是「放弃等待」而非「取消请求」：被放弃的那次请求仍会在后台跑到
> 它自己结束（Python 没有安全的线程取消机制），但命令会及时返回。
