...
| v2.0.0 | 2027 Q4 | 🚧 进行中（TUI、Web 与多币种已提前交付） | 界面与多币种 |

> v0.1.0 与 v0.2.0 的全部内容已随 `0.1.0` 一并交付，v1.0.0 的内容随 `1.0.0` 交付
> （见 [CHANGELOG](../CHANGELOG.md)）。三个里程碑都早于计划时间完成。
>
> **「已发布」指的是打了 git tag，不是传上了 PyPI**——本项目从未上传，
> 文档里的 `pip install 'holdings-cli[…]'` 因此还装不到东西。「指错门」（原先那个
> `holdings` 是别人的同名包）已由 [B-34](BACKLOG.md#b-34) 修掉；**真正把包传上去**
> 是 [B-35](BACKLOG.md#b-35)，**卡在账号而非代码**，仍待办。
...
- [x] Web 界面，只读看板起手（[B-20](BACKLOG.md#b-20)）——持仓 / 报表 /
      净值曲线三页，数据全部来自 `services/`，与 CLI、TUI 共用同一份表格契约。
- [x] 多币种：基准货币人民币，先在汇总里如实排除非 CNY 标的（[B-19](BACKLOG.md#b-19)）
      ——**已提前交付**，汇率换算仍待做。
- [ ] 多账户管理（[B-21](BACKLOG.md#b-21)）。
- [ ] 券商对账单导入，模块化增加不同证券公司（[B-27](BACKLOG.md#b-27) 起 7 项）
      ——**口径与报告**（[B-27](BACKLOG.md#b-27)：非买卖行不入账但逐行报出）与
      **解析器注册表**（[B-28](BACKLOG.md#b-28)：一家券商 = 一个模块 + 注册一行，
      含编码探测与「认不出就报错」）与**导入幂等**
      （[B-29](BACKLOG.md#b-29)：同一份文件导两遍不再翻倍，重复行报出行号）已交付；
      真实券商样本（[B-30](BACKLOG.md#b-30)）待做。
- [ ] 标的资料（名称 / 市场 / 资产类型）的模块化获取（[B-31](BACKLOG.md#b-31)）。
...
- [ ] v2.0.0 进行中：剩余 [B-21](BACKLOG.md#b-21)（多账户）、
      ...
      以及 [B-30](BACKLOG.md#b-30)~[B-33](BACKLOG.md#b-33)（真实券商样本 / 标的资料）。
      [B-27](BACKLOG.md#b-27)（非买卖行的口径与报告）、
      [B-28](BACKLOG.md#b-28)（解析器注册表与自动识别）与
      [B-29](BACKLOG.md#b-29)（导入幂等）已交付。