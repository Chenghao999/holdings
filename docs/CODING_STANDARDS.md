...
| 数据获取层 | `data/` | 封装外部数据源（行情 API、对账单文件），无表现层输出 |
...
1. **BACKLOG 里的一个条目 = 至少一个提交**，提交信息带上条目编号
   ...
   数据源优先级」），拆成多个提交。
2. **配套文档跟代码同一次提交**：改了行为就同时改描述该行为的文档
   （`USER_GUIDE` / `SCHEMA` / `CONFIG_SPEC` / `CHANGELOG` / BACKLOG 条目），
   合在一次提交里。
   **独立**的文档改动（修一处过时描述、改规范本身）才单独成提交。
   这条与「单一目的」并不冲突——「把 X 改成 Y 并把文档改成 Y」本来就是一件事。
   反过来（代码与文档分开提交）会让文档变成一次可以被遗忘的后续动作，
   而这个项目历史上的坑恰恰全是「文档先行、实现没跟上」，不该再给脱节留机会。
