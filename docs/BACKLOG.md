...
**当前状态（2026-09-22）**：除待办的 **B-21~B-23** 与新立项的 **B-27~B-33** 外，
清单内条目**全部交付**（用例 159 → 401，覆盖率 67% → 96%），**v1.0.0 已发布**。
这 10 项都是 v2.0.0 的能力建设，建议顺序见文末
「[剩余条目的执行顺序](#剩余条目的执行顺序)」。
...
| [B-27](#b-27) | P1 | 对账单里认不出的行，不能静默跳过 | `cli/commands/import_cmd.py` |
...
8. **B-27 → B-28 →（B-29 / B-31 并行）→ B-30 → B-32 → B-33** —— 用户提的两条
   ...
   - **B-27 最先**：口径不定，解析器写出来的每一行都可能要重做，而不支持的行
     一旦被静默跳过，用户看到的「导入成功」就是假的；
