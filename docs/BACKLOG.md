...
**当前状态（2026-09-25）**：除待办的 **B-21~B-23**、**B-30~B-33** 与 **B-35** 外，
清单内条目**全部交付**（用例 159 → 488，覆盖率 67% → 96.97%），**v1.0.0 已发布**。
这 8 项均为 v2.0.0 的能力建设，建议顺序见文末
「[剩余条目的执行顺序](#剩余条目的执行顺序)」。
...
| [B-28](#b-28) | ✅ 已完成 | 券商流水导入：解析器注册表与识别 | 新增 `data/brokers/` |
| [B-29](#b-29) | ✅ 已完成 | 同一个文件导两遍，账翻倍 | `storage/`、`services/trade_service.py` |
...
8. **B-27 → B-28 →（B-29 / B-31 并行）→ B-30 → B-32 → B-33** —— 用户提的两条
   ...
   - ~~**B-28 紧随其后**~~ —— ✅ 已完成（2026-09-23）。注册表的形状定下来了
     （一家 = 一个模块 + 四张映射表 + 注册一行），[B-29](#b-29) / [B-31](#b-31) /
     [B-33](#b-33) 都照它写；两家券商本身仍是**示例**格式，真实映射还卡在样本上；
   - ~~**B-29 与 B-31 并行**~~ —— B-29 ✅ 已完成（2026-09-24）。查重与
     `--dedupe` 的口径定了，B-31 与它互不依赖，谁先都行；B-31 是 B-32 的前置；
   - **B-30 卡在样本上**：没有真实对账单，解析器只能对着想象的格式写。
     样本到位前先交付 B-27 / B-28 / B-29（均已交付）/ B-31；
...
两个提交：`data/brokers/` 的骨架、注册表与编码探测，然后 `import_cmd.py` 改走注册表。

**一家券商 = 一个模块 + 四张映射表 + 注册一行。** `matches` 与 `parse` 都留在基类，
子类只填「规范列名 → 本家叫法」「业务名称 → 规范取值」「市场写法 → 市场」
「费用拆在哪几列」，不写任何逻辑——解析器一旦能自由发挥，各家就会各写一套
「什么算一行」「空行怎么办」。加第三家不用改命令行的代码。

**认不出就不猜，像两家也不猜。** 命中必须唯一，两家都像时报「同时符合 A、B，
请用 `--broker` 指定」而不是取第一个：取第一个就是猜，猜错的代价是整批数字错，
而用户会以为导成功了。

**编码顺序反过来是静默的错。** BOM 声明了编码就不再回头猜：截断的 UTF-16 文件
报「读不出内容」，不会退到 GB18030 解出一堆乱码存进账本。

**判据逐条：**

- [x] 两个示例夹具各能整批导入，`--broker` 与自动识别走的是**同一处实现**
      （`tests/test_import_cmd.py`、`tests/test_brokers.py`）。
- [x] 表头陌生 → 「认不出这份对账单的格式，请用 `--broker` 指定」并列出可选项，
      **一笔不写**；同时像两家时同样报错——命中必须唯一。
- [x] GB18030 与 UTF-8 各有用例，BOM / UTF-16 一并锁住；「顺序颠倒会被静默解成
      乱码」这条也有用例指着，不是一个想当然的注释。
- [x] 加第三家券商 = 一个模块 + 注册一行：用例就地造一家塞进 `BROKERS` 即通，
      `import_cmd.py` 一行未改。

**两处补出来的判断**：费用拆成几列时用 `Decimal` 相加——浮点会把
`5.00+1.60+0.02` 存成 `6.619999999999999`，那粒沙会一直跟到成本价；`--fee-column`
照**文件里的写法**给，由解析器在原始表头上解析，优先于该格式自带的费用列。

`demo-a` / `demo-b` 是**示例**格式（代码、文档、报错文案里都这么写），
真实券商的解析器仍卡在样本上（[B-30](#b-30)）。对着想象的格式写映射，
正是这一批条目反复要防的那个错。
...
两个提交：账本加 `source` / `external_id` 两列（含老库补列），然后 `import` 查重
与 `--dedupe`。

**默认报错，而不是跳过，也不是照写。** 条目只要求「报错 + 允许显式跳过」，
实现下来发现第三种情况必须有出口：`--dedupe off`。指纹是
「标的 + 日期 + 类型 + 数量 + 价格 + 费用」，而**同一天分两次、同价同费各买
100 股是真实存在的成交**，指纹分不开——只给 error / skip 两个选项，这份对账单
就永远导不进来（skip 会静默丢掉一笔真成交）。三个取值都是用户明写的，没有静默的。

**同一批文件内部重复的行也查。** 只查库的话，一份自带重复行的导出仍会静默翻倍，
而那与「逐月导出有重叠」是同一件事。

**判重两把尺子**：有券商流水号按 `(source, external_id)`，没有才退到指纹。
标准 CSV 加了可选的 `external_id` 列，券商的解析器把自家那一列映射过来即生效
（`columns` 表加一行，`base.py` 与 `import_cmd.py` 都不用改）。

**`source` 存的是格式标识**（`csv` / `demo-a`，即 `--broker` 的取值），不是人话
名字——改名不该让历史数据的来源对不上。为此 `Statement` 多带一个 `broker_name`。

**判据逐条：**

- [x] 同一文件连导两次：第二次 0 笔入库、指出重复行号、退出码 5
      （`tests/test_import_dedupe.py`，退出码那条走 `main()` 在 `test_cli_errors.py`）。
- [x] 老库（没有这两列）打开后自动补列，已有数据不受影响，补出来的是 NULL
      （`tests/test_storage.py`）。
- [x] 通用 CSV 导入同样受益：上面那批用例走的就是标准 CSV；另有一条走券商格式，
      确认查重在 `import` 这一层、与格式无关。

**顺带**：`--dedupe off` 时不印「重复 N 行」——那是没查，不是没重复；印 0 就成了
谎话。查重时照印「重复 0 行」，「0」是被数出来的。
...
**优先级** P1 · **卡在样本上**——[B-28](#b-28) 的注册表与骨架已就位，接一家券商
只剩「一个模块 + 注册一行」，缺的只有真实样本
...
[B-34](#b-34) 把分发名改成了 PyPI 上没人占的 `holdings-cli`，但**包从未上传**。
文档里的 `pip install 'holdings-cli[…]'` 因此指向一个空名字——比改名前好
