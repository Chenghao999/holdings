...
**当前状态（2026-09-19）**：B-01~B-17 与 B-19 共 18 项**全部交付**
（用例 159 → 380，覆盖率 67% → 95%）。剩余 **B-18、B-20~B-23 共 5 项**，
建议顺序见文末「[剩余条目的执行顺序](#剩余条目的执行顺序)」。其中 B-18（发布收尾）
的最后一个前置条件 B-19 已清掉，可以打 tag 了。
...
| [B-18](#b-18) | 🚧 进行中 | 收尾并发布 v1.0.0 | `pyproject.toml`、`docs/` |
...
B-01~B-17 已交付（原顺序的前两步走完）。以下是 B-18~B-23 的建议顺序，
按「依赖关系 + 能不能立刻做完」排：

1. ~~**B-19**~~ —— ✅ 已完成（2026-09-19）。多币种混加会给出看着像结论的错数，
   与 B-05 / B-06 同类，优先于路线图事项。口径已定（基准货币人民币），
   本轮只要求「如实排除并说明」。
2. **B-18** —— **从这里接着做**。发布收尾：B-19 已清掉那个会误导用户的口径问题，
   可以打 1.0.0 了。发布时需带上 [B-16](#b-16) / [B-17](#b-17) 的两处
   BREAKING CHANGE（`init --dev` 已移除、两个 extra 已删除）。
3. **B-20 → B-21 → B-23** —— v2.0.0 的能力建设，按实际用到的顺序做。
   B-20（Web）与 B-21（多账户）都不依赖彼此，可以先做更常用的那个。
...
**优先级** P1 · 路线图里程碑
**位置** `pyproject.toml`、`CHANGELOG.md`、`ROADMAP.md`、`README.md`

### 现状

`ROADMAP.md` 把 v1.0.0 标为「进行中」，而两批工作（体检 12 项 + 后续 3 项）
已全部交付，功能与测试覆盖都达标。**2026-09-19 起在分支 `b-18-release` 上收尾**
（PR #18），前两步已完成，剩演示与打 tag。
...
1. ✅ **版本号 `1.0.0`，且只有一处**：`pyproject.toml` 的 `version` 是唯一的那个数，
   ...
   安装要重装一次才看得到。
2. ✅ **CHANGELOG 收口**：`Unreleased` → `## [1.0.0] - 2026-09-19`；14 个重复小节
   归成 Added / Changed / Removed / Fixed / Docs 五节（正文逐行保留，只挪位置）；
   删掉已过期的 `### Planned`（其中「清掉 B-01~B-12」早已交付）。

还没做：

3. ⏳ `ROADMAP.md` 的 v1.0.0 勾选收尾（BACKLOG 父项、里程碑总览的「实际状态」）
   ——放到打 tag 那一步一起改，提前打勾就是「说了没做」。
4. ⏳ 演示录制——**卡在工具上**：本机既没有 `terminalizer` 也没有 `asciinema`，
   要先定装哪个，或换个形式（README 里放一份带输出的流程记录）。
5. ⏳ 打 tag `v1.0.0` 并推 CI（仓库当前一个 tag 都没有，这会是第一个）。

### 完成判据

- `holdings --version` 输出 `1.0.0`。
- 打 tag 后 CI 绿。
- README 里有演示动图或链接，`ROADMAP.md` 的 v1.0.0 全部勾上。
