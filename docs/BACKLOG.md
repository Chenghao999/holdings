...
**当前状态（2026-09-17）**：B-01~B-17 共 17 项**全部交付**（16 个 PR 合并，
用例 159 → 360，覆盖率 67% → 95%）。剩余 **B-18~B-23 共 6 项**，
...
1. **B-19** —— **从这里接着做**。多币种混加会给出看着像结论的错数，
   与 B-05 / B-06 同类，优先于路线图事项。口径已定（基准货币人民币），
   本轮只要求「如实排除并说明」。
2. **B-18** —— 发布收尾。**放在 B-19 之后**：先把会给出错数的那项清掉再打 1.0.0，
   否则 tag 上带着一个已知会误导用户的口径问题。发布时需带上
   [B-16](#b-16) / [B-17](#b-17) 的两处 BREAKING CHANGE（`init --dev` 已移除、
   两个 extra 已删除）。
