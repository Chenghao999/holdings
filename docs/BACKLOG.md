...
**当前状态（2026-09-23）**：除待办的 **B-21~B-23**、**B-29~B-33** 与 **B-35** 外，
清单内条目**全部交付**（用例 159 → 466，覆盖率 67% → 96.88%），**v1.0.0 已发布**。
这 9 项均为 v2.0.0 的能力建设，建议顺序见文末
「[剩余条目的执行顺序](#剩余条目的执行顺序)」。
...
8. **B-27 → B-28 →（B-29 / B-31 并行）→ B-30 → B-32 → B-33** —— 用户提的两条
   ...
   - **B-30 卡在样本上**：没有真实对账单，解析器只能对着想象的格式写。
     样本到位前先交付 B-27 / B-28（均已交付）/ B-29 / B-31；
