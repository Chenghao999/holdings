...
**当前状态（2026-09-16）**：B-01~B-15 共 15 项**全部交付**（14 个 PR 合并，
用例 159 → 357，覆盖率 67% → 94%）。剩余 **B-16~B-23 共 8 项**，
建议顺序见文末「[剩余条目的执行顺序](#剩余条目的执行顺序)」。
...
B-01~B-15 已交付（原顺序的 6 步全部走完）。以下是 B-16~B-23 的建议顺序，
按「依赖关系 + 能不能立刻做完」排：

1. **B-16 → B-17** —— 两处「说了没做」，改动都很小、各一个提交。
   **先做这两件**：它们与刚清完的那批同类，留着就是下一轮体检的原料。
2. **B-19** —— 多币种混加会给出看着像结论的错数，与 B-05 / B-06 同类，
   优先于路线图事项。口径已定（基准货币人民币），本轮只要求「如实排除并说明」。
3. **B-18** —— 发布收尾。**放在 1、2 之后**：先把「说了没做」清掉再打 1.0.0，
   否则 tag 上会带着两处已知的空承诺。
4. **B-20 → B-21 → B-23** —— v2.0.0 的能力建设，按实际用到的顺序做。
   B-20（Web）与 B-21（多账户）都不依赖彼此，可以先做更常用的那个。
5. **B-22** —— 桌面端形态未定，等 B-16 的结论（删 extra 还是标预留）再决定。
...
已全部交付，功能与测试覆盖都达标。但**版本号还停在 `0.1.0`**，CHANGELOG 的
内容也堆在 `Unreleased` 下。

### 要做什么

1. `pyproject.toml` 的 `version` 提到 `1.0.0`。
2. `CHANGELOG.md` 把 `Unreleased` 收成 `[1.0.0] - <日期>`。
3. `ROADMAP.md` 的 v1.0.0 勾选收尾（BACKLOG 父项、里程碑总览的「实际状态」）。
4. 用 `terminalizer` 或 `asciinema` 录一段演示（`init` → `add` → `sync` →
   `list` → `report` → `tui`），放进 README。
5. 打 tag 并推 CI。
...
**优先级** P2 · 会误导用户
**位置** `services/portfolio_service.py`、`utils/formatter.py`
