...
- **导入幂等**（[BACKLOG B-29](docs/BACKLOG.md)）：`import` 落库前查重，发现
  已经记过的行**默认报出行号、一笔不写**（退出码 `5`）。逐月补充导出时月度文件
  之间有重叠是常事，此前重导一次就把持仓数量与成本价翻一倍，且看不出来。
  `--dedupe skip` 跳过重复的、`off` 不查重（同一天同价同费的**两笔真实成交**
  需要它，指纹分不开那种情况）。
  - 判重优先用**券商流水号**（标准 CSV 的 `external_id` 列，券商的解析器把自家
    那一列映射过来即生效）；没有则退回**全字段指纹**：标的 + 日期 + 类型 +
    数量 + 价格 + 费用。
  - `transactions` 表新增 `source`（来源格式名）与 `external_id` 两列，老库
    打开时自动补列（走 `_migrate`，用户不需要做任何事）。
  - 导入汇总行随之多一截：`识别 N 行，入账 M 笔，未入账 K 行，重复 J 行`。
...
- **三个界面共用一份表格契约**（[BACKLOG B-20](docs/BACKLOG.md)，顺带的结构调整）：
  ...
  都没有理由各写一遍。
- **`import` 不再对账单里认不出的行睁一只眼闭一只眼**（[BACKLOG B-27](docs/BACKLOG.md)）：
  真实对账单里的分红派息、送股转增、配股、银证转账、利息这五类行，本工具
  表达不了（前三类要改成本价，口径见 [B-23](docs/BACKLOG.md)；后两类动的是
  现金），改动前它们会让**整批导入失败**并只说一句「不是合法交易类型」。
  现在这三种结局分得开：
  - **入账**：`BUY` / `SELL` / `FEE`，与改动前一致；
  - **不入账但逐行报出来**：上面那五类，汇总固定为
    `识别 N 行，入账 M 笔，未入账 K 行`，每条未入账的行带行号、原始写法与原因。
    这些行不做逐列校验——分红行常常没有数量与价格；
  - **整批拒绝、退出码 5**：其余一切认不出的值，与改动前一致。
    这一条是刻意留的：把认不出的值也当成「不入账」放过，用户改错了文件
    却什么都看不到。
  - 新增 `--strict`：有未入账的行时**一笔都不写**、退出码 `5`（默认 `0`）。
    之所以不是「先写进去再报错」，是因为 `import` 还没有幂等
    （[B-29](docs/BACKLOG.md)）——写了一半又以非零退出码结束，重跑一次就把账翻倍。
  分类表与用户会看到的输出写进了 `USER_GUIDE.md` 第 5 节。
- **对账单不再只认一种格式**（[BACKLOG B-28](docs/BACKLOG.md)）：此前 `import`
  写死了「英文表头 + UTF-8」这一种 CSV，而券商导出的对账单在四个维度上都不同
  ——编码（GBK）、表头（中文且各家叫法不同）、交易类型（`证券买入` / `申购`…）、
  费用（佣金 / 印花税 / 过户费拆三列）。现在新增 `src/holdings/data/brokers/`：
  - 一家券商一个模块，实现同一套接口（`name` / `matches` / `parse`）；
    **加一家券商 = 一个模块 + 在注册表里加一行**，`import_cmd.py` 不改。
  - `--broker <名字>` 显式指定，不给则按表头自动识别；**认不出、或同时像两家
    都报错，不猜**——猜错的代价是整批数字错，而用户会以为导成功了。
  - 编码按 `BOM → UTF-8 → GB18030` 探测，GBK 的对账单不用先转码。
  - 随本次交付的还有标准 CSV 格式（与改动前完全兼容）与两个**示例**格式夹具；
    真实券商的解析器等真实样本（[B-30](docs/BACKLOG.md)）。
  - `data/` 的定义随之从「只封装外部 API」改为「外部数据的适配器：API 与文件格式」。
- **补上 `holdings chart` 产图路径的用例**（[BACKLOG B-25](docs/BACKLOG.md)）：
  `chart.py` 最后三行（写文件 / 告知写到哪儿 / 请浏览器打开）此前在测试里
  零执行，用例只覆盖「未来日期」与「非法日期」两条错误路径——图**算**得对不对
  有人守着，**落盘与通知**那一段没有任何东西看着。新用例跑完整命令，断言产物
  真是 plotly 图且含快照日期；`webbrowser.open` 打桩，否则本地跑一次测试
  就弹一个浏览器窗口。产图链路三个文件均达 100%。
  这一项是 B-24 的直接产物：CI 的 `test` 作业开始装 `.[dev,chart]`，plotly
  才真的在场，否则新用例只会是一条天天被跳过的用例。
...
- `--start` 的 help 自己就写着「暂存参数」，`networth_figure()` 也没有对应形参——
  传了完全不生效，而 `USER_GUIDE` 却拿它当日期筛选的示例。现真正接线：
  只保留该日（含）之后的快照，`--start 2025-13-45` 这类非法日期返回退出码 5。
- **起始日期之后没有数据时报错，而不是生成空白图**：空图与「净值跌没了」在图上
  分不出来。提示里带上现有的数据范围（条数与首末日期），用户才知道该把 `--start`
  调到哪里。为此把取数从 `networth_figure` 里拆成 `networth_series`，
  它不依赖 plotly——没装画图库时数据问题也能先报出来。
- 日期参数的回调收进 `cli/dates.py` 共用：此前只有 `add` 有一份，`chart` 没校验。
